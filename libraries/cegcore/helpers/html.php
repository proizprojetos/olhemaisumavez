	...
	public static function html_formLine($name, $params = array()){
		...
		switch($params['type']){
			default:
				...
				$tags[] = self::container(self::set_tag(1), $form_cell, array('class' => 'gcore-line-tr', 'id' => 'ftr'.$postfix));
				break;
		}
		return implode("\n", $tags);
	}
	...
	public static function html_input($name, $params = array()){
		...
		switch($params['type']){
			...
			case 'reset':
				...
				break;
			case 'textarea':
				$attributes = array('name', 'id', 'class', 'rows', 'cols', 'title', 'style', 'onclick', 'onchange', 'alt', 'placeholder');
			...
			case 'text':
			case 'password':
				$attributes = array('type', 'name', 'id', 'value', 'class', 'size', 'maxlength', 'title', 'style', 'onclick', 'onchange', 'alt', 'placeholder');
				$params['id'] = self::_field_uid($params['id']);
				$tags[] = self::_concat($params, $attributes, '<input ', ' />');
				break;
			case 'multi':
