...
class Readability
{
	...
	*/
	protected function preClean() {
		...
		$parts = $this->fgParams->getValue('remove_by_attrib');
			$parts = explode(',',$parts);
	...
	**/
	...
	{
		$this->debug = $this->fgParams->getValue('debug'); //FG edit

		if(strpos($this->fgParams->getValue('strip_list'),'script') === false) $this->removeScripts($this->dom);
		...
		$relaxParsing	= $this->fgParams->getValue('relax_parsing'); //FG edit
	...
	*/
	function prepArticle($articleContent) {
		...
		if(strpos($this->fgParams->getValue('strip_list'),'embed') === false) $this->clean($articleContent, 'embed');
		if(strpos($this->fgParams->getValue('strip_list'),'object') === false) $this->clean($articleContent, 'object');
		if(strpos($this->fgParams->getValue('strip_list'),'iframe') === false) $this->clean($articleContent, 'iframe');
		...
		***/
		...
		for ($i = $articleParagraphs->length-1; $i >= 0; $i--)
		{
			...
			$embedCount  = strpos($this->fgParams->getValue('strip_list'),'embed') ? $articleParagraphs->item($i)->getElementsByTagName('embed')->length : 0;
			$objectCount = strpos($this->fgParams->getValue('strip_list'),'object') ? $articleParagraphs->item($i)->getElementsByTagName('object')->length : 0;
			$iframeCount = strpos($this->fgParams->getValue('strip_list'),'iframe') ? $articleParagraphs->item($i)->getElementsByTagName('object')->length : 0;
	...
	//	$nodesToScore = array();
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			if ($grabLikelyCandidates) {
				if(strpos($this->regexps['likelyImageCandidates'],'x/i') AND $likelyCandidates = $this->fgParams->getValue('likelyCandidates','')) {
	...
	**/
	...
	//	$nodesToScore = array();
		...
		//for ($nodeIndex=$targetList->length-1; $nodeIndex >= 0; $nodeIndex--) {
			...
			if ($grabLikelyCandidates) {
				if(strpos($this->regexps['likelyVideoCandidates'],'x/i') AND $likelyCandidates = $this->fgParams->getValue('likelyVideoCandidates','')) {
					$likelyCandidates = explode(',',$likelyCandidates);
				...
				{
					$videoList = array();
					if(strpos($this->fgParams->getValue('strip_list'),'embed')) $videoList[] = $node->getElementsByTagName('embed');
					if(strpos($this->fgParams->getValue('strip_list'),'object')) $videoList[] = $node->getElementsByTagName('object');
					if(strpos($this->fgParams->getValue('strip_list'),'iframe')) $videoList[] = $node->getElementsByTagName('iframe');
